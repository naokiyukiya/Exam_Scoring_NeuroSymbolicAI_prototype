          ...
          parts: [
            { inlineData: { mimeType: 'image/jpeg', data: base64Image } },
            {
              text: `
                ...
                [抽出ルール（厳守）]
                1. グラフの基本構造と終端:
                   - メインの論理フローは必ず「命題」→「推論」→「命題」→「推論」と交互に配置し、**グラフの最後のノードは必ず「命題（proposition）」で終了してください。**
                   - 命題ノード同士、または推論ノード同士が直接繋がることは禁止します。
                2. ハルシネーション（勝手な捏造）の絶対禁止:
                   - **答案画像にインクとして実際に書かれている数式・文字のみを命題ノードとして正確に抽出してください。**
                   - 省略されている途中式や、解説のためにAIが独自に補完した「書かれていない数式・命題」を勝手に生成・追加することは固く禁じます。答案に存在しない式を勝手に作ってはいけません。
                3. 定理（theorem）ノードの抽出と接続:
                   - Σ（シグマ）記号を使った和の計算や公式変形が含まれる場合、対応する定理ノード（type: "theorem"）を生成し、対応する推論ノードから 'edges' で確実に接続してください。定理ノードを画面左側に孤立させないこと。
                4. 推論ノードの検証データ付与:
