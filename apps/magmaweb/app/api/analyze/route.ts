...
3. 【推論ノードのラベルの具体化（超重要）】:
   ...
   - 代わりに、**「命題p11の条件（x < 2）を考慮し、命題p4の両辺に負の数 (x-2) を掛けて不等号の向きを反転させ、命題p12を導く」**のように、具体的にどの命題に対してどのような式変形や条件適用を行ったのかを詳細かつ分かりやすく日本語で記述してください。
4. 複数の命題の組み合わせ:
