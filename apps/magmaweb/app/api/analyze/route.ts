...
// ★ 追加: Next.js (Vercel等) のAPIタイムアウト制限をデフォルト(15秒)から60秒に延長する
export const maxDuration = 60;

// ★ プロンプトのバージョン
const PROMPT_VERSION = "1.10.0";
          ...
          parts: [
            { inlineData: { mimeType: 'image/jpeg', data: base64Image } },
            {
              text: `
                ...
                [抽出ルール]
                1. グラフの基本構造（【絶対遵守】厳密な交互配置と完走）:
                   - メインの論理フローは、必ず「命題」→「推論」→「命題」→「推論」と厳密に交互に繋がるように配置してください。
                   - 【重要】命題ノード同士、または推論ノード同士が直接繋がることは絶対に禁止します。
                   - 【超重要】ステップ数が多くなった場合でも、途中で生成を省略したり打ち切ったりすることは絶対に禁止します。必ず最後の結論の命題ノードまで書き切り、JSONの閉じカッコが完了するまで出力を続けてください。グラフの終端は必ず「命題」ノードにしてください。
                ...
                3. 推論（inference）ノードと定義・定理（theorem）ノードの接続ルール（基本と特例）:
                   ...
                   - 【特例ルール（命題からの直接接続）】もし推論ノードの内容が公式変換に直接関係ない場合でも、命題の数式内に「Σ（シグマ）」などの重要な定義・定理が含まれている場合は、特例として【命題ノードから直接、定義・定理ノードへエッジを繋ぐ】ことを強く推奨します。
